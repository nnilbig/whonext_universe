// ============================================
// 【已停用 2026-08-10】profiles 分頁已經被 players 分頁取代（line_user_id
// 當主鍵，一定要先用 LINE 登入過才會有列，不能再放「舊名冊、還沒登入」
// 的佔位列），這支腳本引用的 PROFILES_SHEET 常數在 Code.gs 已經改名成
// PLAYERS_SHEET，直接執行會噴錯。它原本要做的事（列出還沒綁定的舊球員
// 姓名）已經被 Code.gs 的 getRoster() 取代，即時從 members 撈，不用再
// 跑這支一次性腳本。留著只是歷史紀錄，不要執行。
// ============================================
//
